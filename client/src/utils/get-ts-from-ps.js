...
export const getTsFromPs = (ps) => {
  if (ps <= 611.213 * 10 ** -6) {
    throw Error('Давление должно быть больше 611.213 Pa');
  }

  if (ps >= 22.064) {
    throw Error('Давление должно быть меньше  22.064 MPa');
  }
  ...
  const result = t8 * ((n[10] + D - Math.sqrt(((n[10] + D) ** 2) - 4 * (n[9] + n[10] * D))) / 2);
